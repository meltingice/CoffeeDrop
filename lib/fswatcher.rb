...
class FSWatcher
  ...
  def initialize()
    ...
    @fsevent = FSEvent.new(RubyDrop.config['rubydrop_root'])
  end
  ...
  def start
    Dir.chdir(RubyDrop.config['rubydrop_root']) do
      ...
      #    
      #    # need to remove the reset and handle conflicts by renaming
